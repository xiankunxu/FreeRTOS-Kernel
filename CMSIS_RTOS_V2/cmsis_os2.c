...
*/
/* Comment out this API to avoid compiler warning of implicit declaration of function 'OS_Tick_GetCount'
 * This API is not called by the application */
...
osMutexId_t osMutexNew (const osMutexAttr_t *attr) {
  ...
  if (IRQ_Context() == 0U) {
    ...
    if ((type & osMutexRobust) != osMutexRobust) {
      ...
      /* TODO: delete below to make mutex not registered by default.
       * Alternatively, pass attr = NULL to disable registry */
...
osStatus_t osMutexAcquire (osMutexId_t mutex_id, uint32_t timeout) {
  ...
  if (IRQ_Context() != 0U) {
    /* NOTE: ISR context is not allowed to acquire mutex in this implementation */
    stat = osErrorISR;
  }
  else if (hMutex == NULL) {
    stat = osErrorParameter;
...
osStatus_t osMutexRelease (osMutexId_t mutex_id) {
  ...
  /* NOTE: not able to release mutex in ISR context */
  if (IRQ_Context() != 0U) {
    stat = osErrorISR;
  }
  else if (hMutex == NULL) {
...
osSemaphoreId_t osSemaphoreNew (uint32_t max_count, uint32_t initial_count, const osSemaphoreAttr_t *attr) {
  ...
  if ((IRQ_Context() == 0U) && (max_count > 0U) && (initial_count <= max_count)) {
    ...
    if (mem != -1) {
      ...
      /* TODO: delete below to disallow semaphore registration */
      #if (configQUEUE_REGISTRY_SIZE > 0)
      if (hSemaphore != NULL) {
        if ((attr != NULL) && (attr->name != NULL)) {
          /* Only non-NULL name objects are added to the Queue Registry */
...
osStatus_t osSemaphoreAcquire (osSemaphoreId_t semaphore_id, uint32_t timeout) {
  ...
  if (hSemaphore == NULL) {
    ...
    if (timeout != 0U) {
      ...
      /* NOTE: semaphore can be taken in ISR context with timeout==0 */
...
*/
/* TODO: add register function to record memory pool, which can then be used to query the high water mark of the memory pool */
        ...
        if (timeout == 0U) {
          if (xSemaphoreTakeFromISR (mp->sem, NULL) == pdTRUE) {
            if ((mp->status & MPOOL_STATUS) == MPOOL_STATUS) {
              ...
              // TODO mp set a minimum value of available blocks, 
              // if n < exist, then set exit to n
